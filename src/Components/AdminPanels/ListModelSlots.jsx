...
export default function ListModelSlots(props) {
    ...
    // We need to take the selected environment and see the number of models it has, then create an empty div for each of them

    const [slots, setSlots] = useState([])

    // GOAL 1: Get environment (API Call with props.environmentId)
    ...
    // GOAL 2: Read exhibits field

    // GOAL 2.5: API endpoint to get all objects in an environment (convert list of IDs into list of objects, including null)

    // GOAL 3: One div per modelSlot and one div per panelSlot (showing the index)

    // GOAL 4: Placeholder object per modelSlot and panelSlot (showing the index)

    // GOAL 5: Drag and drop from model list to slot

    // GOAL 6: Update DB info with new object
